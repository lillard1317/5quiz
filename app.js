  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0f0f1a", fontFamily:"-apple-system,Hiragino Sans,sans-serif", color:"#eee" }}>
      ...
      <div style={{ padding:"16px 16px 100px" }}>
        ...
        {(() => {
          const choices = rh.shuffledChoices || rh.question.choices.map((text,i)=>({text,origIndex:i}));
          const corIdx = rh.shuffledChoices ? rh.shuffledChoices.findIndex(x => x.origIndex === rh.question.answer-1) : rh.question.answer-1;
          const selChoice = choices[rh.selected];
          const corChoice = choices[corIdx];
          const isCorrect = rh.correct;

          if (isCorrect) {
            return (
              <div style={{ marginBottom:4 }}>
                <div style={{ fontSize:11, color:"#2ecc71", fontWeight:700, marginBottom:6, letterSpacing:1 }}>正解</div>
                <div style={{ padding:"14px 16px", borderRadius:12, fontSize:14, background:"rgba(46,204,113,0.15)", border:"2px solid #2ecc71", color:"#2ecc71", display:"flex", alignItems:"flex-start", gap:12 }}>
                  <span style={{ flex:1, lineHeight:1.7 }}>{corChoice.text}</span>
                </div>
              </div>
            );
          }
          ...
          return (
            <div style={{ marginBottom:4 }}>
              <div style={{ fontSize:11, color:"#e74c3c", fontWeight:700, marginBottom:6, letterSpacing:1 }}>あなたが選択したもの</div>
              <div style={{ padding:"14px 16px", borderRadius:12, marginBottom:16, fontSize:14, background:"rgba(231,76,60,0.15)", border:"2px solid #e74c3c", color:"#e74c3c", display:"flex", alignItems:"flex-start", gap:12 }}>
                <span style={{ flex:1, lineHeight:1.7 }}>{selChoice.text}</span>
              </div>
              <div style={{ fontSize:11, color:"#2ecc71", fontWeight:700, marginBottom:6, letterSpacing:1 }}>正解</div>
              <div style={{ padding:"14px 16px", borderRadius:12, fontSize:14, background:"rgba(46,204,113,0.15)", border:"2px solid #2ecc71", color:"#2ecc71", display:"flex", alignItems:"flex-start", gap:12 }}>
                <span style={{ flex:1, lineHeight:1.7 }}>{corChoice.text}</span>
        ...
        <div style={{ borderRadius:16, overflow:"hidden", marginTop:8 }}>
          <div style={{ background: rh.correct ? "rgba(46,204,113,0.2)" : "rgba(231,76,60,0.2)", borderBottom: `2px solid ${rh.correct ? "#2ecc71" : "#e74c3c"}`, padding:"12px 16px", display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ fontSize:15, fontWeight:900, color: rh.correct ? "#2ecc71" : "#e74c3c" }}>
              {rh.correct ? "正解" : "不正解"}
            </span>
            <span style={{ fontSize:12, color:"#aaa", marginLeft:"auto" }}>解説</span>
          </div>
          <div style={{ background:"rgba(255,255,255,0.05)", padding:"16px", border:"1px solid rgba(255,255,255,0.08)", borderTop:"none", borderRadius:"0 0 16px 16px" }}>
            <p style={{ fontSize:14, lineHeight:2.0, color:"#ddd", margin:0, whiteSpace:"pre-wrap" }}>{rh.question.explanation}</p>
          </div>
        </div>
      </div>
...
function ResultScreen({ history, score, totalQ, onRetry, onReview }) {
  ...
  const gt = percent >= 80 ? "合格圏内！" : percent === 60 ? "ギリギリ合格！" : percent > 60 ? "合格圏内！" : "要復習";
...
function App() {
  ...
  const handleStart = (cats, num) => {
    ...
    setHistory([]); setScore(0); changeMode("quiz");
  };

  const handleFinish = (hist, finalScore) => {
    setHistory(hist); setScore(finalScore); changeMode("result");
  };
  ...
  const tabBar = (
    <TabBar
      onReference={() => changeMode("reference")}
      onQuiz={() => changeMode("menu")}
      onSettings={() => changeMode("settings")}
    />
  );

  if (mode === "reference") return <><ReferenceScreen onBack={() => changeMode("menu")} />{tabBar}</>;
  if (mode === "settings") return <><SettingsScreen onBack={() => changeMode("menu")} allQCount={allQuestions.length} />{tabBar}</>;
  if (mode === "menu") return <><MenuScreen allQuestions={allQuestions} onStart={handleStart} />{tabBar}</>;
  if (mode === "quiz") return <QuizScreen questions={questions} onFinish={handleFinish} onMenu={() => changeMode("menu")} />;
  if (mode === "result") return <ResultScreen history={history} score={score} totalQ={questions.length} onRetry={() => changeMode("menu")} onReview={(i) => { setReviewStartIdx(i); changeMode("review"); }} />;
  if (mode === "review") return <ReviewScreen history={history} startIdx={reviewStartIdx} onBack={() => changeMode("result")} />;
  return null;
}
