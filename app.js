  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#1a1a2e,#16213e,#0f3460)", fontFamily:"-apple-system,Hiragino Sans,sans-serif", padding:"24px 20px 100px" }}>
      <div style={{ maxWidth:460, margin:"0 auto" }}>
        ...
        {/* カテゴリ選択 */}
        <div style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:18, marginBottom:24, overflow:"hidden", marginTop:32 }}>
        ...
        {/* 出題数スライダー */}
        <div style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:18, padding:"18px 16px", marginBottom:24 }}>
        ...
        <button onClick={() => onStart(selected, safeNum)}
          style={{ width:"100%", padding:18, borderRadius:14, border:"none", background:"linear-gradient(135deg,#e74c3c,#c0392b)", color:"#fff", fontSize:18, fontWeight:900, cursor:"pointer", letterSpacing:3, boxShadow:"0 6px 24px rgba(231,76,60,0.45)", marginBottom:16 }}>
          START
        </button>
      </div>
    </div>
...
function QuizScreen({ questions, onFinish }) {
  ...
  const isCorrect = answered && selected + 1 === q.answer;
    ...
    <div style={{ minHeight:"100vh", background:"#0f0f1a", fontFamily:"-apple-system,Hiragino Sans,sans-serif", color:"#eee" }}>
      ...
      <div style={{ padding:"16px 16px 32px", paddingBottom: answered ? 32 : 90 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:12 }}>
          {q.choices.map((c, i) => {
            const isCor = i+1 === q.answer, isSel = i === selected;
...
function ReviewScreen({ history, startIdx, onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0f0f1a", fontFamily:"-apple-system,Hiragino Sans,sans-serif", color:"#eee" }}>
      <div style={{ background:"rgba(255,255,255,0.03)", borderBottom:"1px solid rgba(255,255,255,0.07)", padding:"12px 16px", display:"flex", alignItems:"center", gap:12 }}>
      ...
      <div style={{ padding:"16px 16px 100px" }}>
        ...
        {rh.question.choices.map((c, i) => {
          const isCor = i+1 === rh.question.answer, isSel = i === rh.selected;
          return (
...
function ResultScreen({ history, score, totalQ, onRetry, onReview }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#1a1a2e,#16213e,#0f3460)", fontFamily:"-apple-system,Hiragino Sans,sans-serif", color:"#eee", padding:"24px 20px", overflowY:"auto" }}>
      <div style={{ maxWidth:460, margin:"0 auto" }}>
...
function ReferenceScreen({ onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0a0a14", fontFamily:"-apple-system,Hiragino Sans,sans-serif", color:"#eee", paddingBottom:80 }}>
      <div style={{ background:"rgba(255,255,255,0.04)", borderBottom:"1px solid rgba(255,255,255,0.08)", padding:"14px 16px", display:"flex", alignItems:"center", gap:10, position:"sticky", top:0, zIndex:10, backdropFilter:"blur(10px)" }}>
        <button onClick={onBack} style={{ background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.15)", color:"#fff", padding:"8px 14px", borderRadius:10, cursor:"pointer", fontSize:13, fontWeight:700 }}>← 戻る</button>
        <div>
