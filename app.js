      ...
      <div style={{ padding:"16px 16px 100px" }}>
        ...
        {(() => {
          ...
          return choices.map((c, i) => {
            const isCor = i === corIdx;
            const isSel = i === rh.selected;
            if (!isCor && !isSel) return null;
            return (
              <div key={i} style={{ padding:"14px 16px", borderRadius:12, marginBottom:10, fontSize:14, background:isCor?"rgba(46,204,113,0.15)":"rgba(231,76,60,0.15)", border:`2px solid ${isCor?"#2ecc71":"#e74c3c"}`, color:isCor?"#2ecc71":"#e74c3c", display:"flex", alignItems:"flex-start", gap:12 }}>
                <span style={{ fontSize:16, minWidth:22, marginTop:1 }}>{isCor ? "[正]" : "[誤]"}</span>
                <span style={{ flex:1, lineHeight:1.7 }}>{c.text}</span>
              </div>
            );
          });
        })()}
